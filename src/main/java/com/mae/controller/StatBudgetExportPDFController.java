 ...
 * @author hp
 */
public class StatBudgetExportPDFController {
        ...
        if (maSelection == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                .setMultipliedLeading(2.5f); // Interligne de 2.5x
                 document.add(interligne);
 
                // Ajout d'un titre
                Paragraph title = new Paragraph("RAPPORT BUDGETAIRE")
                        .setFont(police)
                        .setFontSize(16)
                ...
                .setMultipliedLeading(2.5f); // Interligne de 2.5x
                 document.add(interligne2);
            
            /**********************************TABLEAU DU DONNEES*************************************************************************/            
                ///tableau de données
                int columnCount = InterfaceStatistiqueBudget.tableau_agent.getColumnCount();
                PdfFont policeContenu = PdfFontFactory.createFont(StandardFonts.COURIER);
                float[] largeurtableauDeDonnees = {50, 100, 200, 400, 50, 100, 50, 50 };
                Table pdfTable = new Table(largeurtableauDeDonnees);
                pdfTable.setAutoLayout();               

                // Ajouter les en-têtes
                for (int i = 0; i < columnCount; i++) {
                    pdfTable.addCell(InterfaceStatistiqueBudget.tableau_agent.getColumnName(i)).setFont(policeContenu).setTextAlignment(TextAlignment.CENTER);
                ...
                // Ajouter les données
                for (int i = 0; i < InterfaceStatistiqueBudget.tableau_agent.getRowCount(); i++) {
                    for (int j = 0; j < columnCount; j++) {
                        Object value = InterfaceStatistiqueBudget.tableau_agent.getValueAt(i, j);
                        pdfTable.addCell(value != null ? value.toString() : "").setFont(policeContenu);
                    }
                }       
                
                //ajout d'interligne
                 Paragraph interligne3 = new Paragraph("")
                .setMultipliedLeading(2.5f); // Interligne de 2.5x
                 document.add(interligne3);
               /**********************************TABLEAU TOTAL BUDGET*************************************************************************/   
                int columnCountTotalBudget = InterfaceStatistiqueBudget.tableau_total.getColumnCount();
                PdfFont policeContenuTotalBudget = PdfFontFactory.createFont(StandardFonts.COURIER);
                float[] largeurtableauDeDonneesTotalBudget = {500, 500 };
                Table pdfTableTotalBudget = new Table(largeurtableauDeDonneesTotalBudget);
                pdfTableTotalBudget.setAutoLayout();  
                
                 // Ajouter les en-têtes
                for (int i = 0; i < columnCountTotalBudget; i++) {
                    pdfTableTotalBudget.addCell(InterfaceStatistiqueBudget.tableau_total.getColumnName(i)).setFont(policeContenu).setTextAlignment(TextAlignment.CENTER);
                 //   pdfTable.addHeaderCell(new Cell().add(InterfaceStatistiqueAgent.tableau_agent.getColumnName(i)).setBackgroundColor(ColorConstants.LIGHT_GRAY));
                }
                
                // Ajouter les données
                for (int i = 0; i < InterfaceStatistiqueBudget.tableau_total.getRowCount(); i++) {
                    for (int j = 0; j < columnCountTotalBudget; j++) {
                        Object value = InterfaceStatistiqueBudget.tableau_total.getValueAt(i, j);
                        pdfTableTotalBudget.addCell(value != null ? value.toString() : "").setFont(policeContenu);
                    }
                }                            
                ...
                document.add(interligne3);
                // Ajout d'un titre
                Paragraph title2 = new Paragraph("RECAPITULATIF DES TOTAUX")
                        .setFont(police)
                        .setFontSize(16)
                        .setTextAlignment(TextAlignment.CENTER)
                        .setUnderline()
                ...
                //  .setTextAlignment(TextAlignment.CENTER);
                document.add(title2);
                document.add(pdfTableTotalBudget);
                document.close();
